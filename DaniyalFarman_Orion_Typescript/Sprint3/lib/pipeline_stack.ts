...
export class DaniyalPipeline extends Stack {
    constructor(scope: Construct, id: string, props?: StackProps) {
        ...
        const source = pipelines.CodePipelineSource.gitHub('daniyal2022skipq/Orion_Python', 'main', {
        ...
        const synth = new pipelines.ShellStep('Synth', {
            input: source,
            commands: ['cd DaniyalFarman_Orion_Typescript/Sprint3', 'npm ci', 'npm run build', 'npx cdk synth'],
            primaryOutputDirectory: 'DaniyalFarman_Orion_Typescript/Sprint3/cdk.out',
        });

        const unitTest = new pipelines.ShellStep('UnitTests', {
            commands: ['cd DaniyalFarman_Orion_Typescript/Sprint3', 'npm ci', 'npm run test']
        });

        const daniyalPipeline = new pipelines.CodePipeline(this, 'DaniyalFarmanPipeline', { synth: synth });
